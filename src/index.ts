...
async function handleInteractionCommands(
  ...
  else if (commandName === "price-cg") {
    const tokenId = "scout-protocol-token";
    try {
      const tokenData = await fetchTokenPrice(tokenId);
      if (tokenData && typeof tokenData.usd === "number") {
        await interaction.reply(
          `**DEV Token Price (CoinGecko):** $${tokenData.usd.toFixed(5)}`
        );
      } else {
        await interaction.reply(
          "Sorry, couldn't fetch the price from CoinGecko right now. Please try again later."
        );
      }
    } catch (error) {
      logger.error("Error fetching DEV price from CoinGecko", error);
      await interaction.reply(
  ...
  try {
    ...
    for (const command of commandsToDelete) {
      try {
        ...
        if (error.code === 10063) {
          logger.warn(
            `Command ${command.name} already deleted or doesn't exist, skipping`
          );
        } else {
